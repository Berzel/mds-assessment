    ...
    <div class="container mt-2">
        ...
        <form class="d-block mt-4 w-md-50" method="POST" action={{ route('tasks.update', ['task' => $task]) }}>
            ...
            <div class="mb-4">
                ...
                <input type="text" class="form-control" id="title" value="{{ old('title') ?? $task->title }}" name="title">
                @error('title')
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control" id="description" rows="4" name="description">{{ old('description') ?? $task->description }}</textarea>
                @error('description')
                    <small class="form-text text-danger">{{ $message }}</small>
                @enderror
            </div>
